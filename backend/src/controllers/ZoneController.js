...
const ZoneController = {
  ...
  async createZone(req, res) {
    try {
      ...
      return res.status(201).json({ success: true, data: { zone: newZone }, message: "Successfuly add zone" });
    } catch (error) {
      return res.status(500).json({ success: false, message: error.message });
    }
  },
  ...
  async updateZone(req, res) {
    try {
      ...
      if (kapasitas !== undefined && Number(kapasitas) < existing.kapasitas_terisi) {
        return res.status(422).json({
          success: false,
          message: `Maximum capacity must be greater than the current filled capacity (${existing.kapasitas_terisi})`,
        });
      }
      ...
      if (kode_zona || nama_zona) {
        const duplicate = await ZoneModel.findDuplicate({
          ...
          nama_zona: nama_zona || existing.nama_zona,
          excludeId: id,
        });
        if (duplicate) {
          return res.status(409).json({ success: false, message: "Name or Zone Code already used" });
        }
      }

      // Fall back to the existing values for any field not provided, so a
      // partial update never wipes out the columns the client didn't send.
      const updated = await ZoneModel.update(id, {
        kode_zona: kode_zona !== undefined ? kode_zona : existing.kode_zona,
        nama_zona: nama_zona !== undefined ? nama_zona : existing.nama_zona,
        kapasitas: kapasitas !== undefined ? Number(kapasitas) : existing.kapasitas,
        deskripsi: deskripsi !== undefined ? deskripsi : existing.deskripsi,
      });

      return res.status(200).json({ success: true, message: "successfully update zone", data: { zone: updated } });
    } catch (error) {
      return res.status(500).json({ success: false, message: error.message });
    }
  },
  ...
  async deleteZone(req, res) {
    try {
      ...
      const occupied = await ZoneModel.hasItem(req.params.id);
      if (occupied) {
        return res.status(409).json({
          success: false,
          message: "Zone cannot be deleted because it still contains items",
        });
      }
